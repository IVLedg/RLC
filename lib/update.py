...
def update():
    if not os.path.exists(os.path.join(ROOTDIR, ".git")):
        msg = "[-] Not a git repository. Please checkout the repository from GitHub (e.g. git clone https://github.com/IVLedg/RLC.git)"
        logger.error(msg)
    if PLATFORM == 'nt':
        msg = "[-] Please checkout the repository from GitHub with GitHub for Windows (e.g. https://windows.github.com)"
        logger.warning(msg)
        msg = "[*] Repository at https://github.com/IVLedg/RLC.git"
        logger.info(msg)
    else:
        msg = "[*] Updating RLC from latest version from the GitHub Repository\n" 
        logger.info(msg)
